    ...
    return Controller.extend("ns.backorder.controller.Main", {
        ...
        onSearch: function () {
            ...
            if (sSearchValue) {
                ...
                aFilters.push(new Filter(sTargetField, FilterOperator.Contains, sSearchValue));
            }
            ...
            if (oFromDate) {
                var sFrom = oFromDate.getFullYear() + "-" + 
                            String(oFromDate.getMonth() + 1).padStart(2, '0') + "-" + 
                            String(oFromDate.getDate()).padStart(2, '0');
                aFilters.push(new Filter("orderDate", FilterOperator.GE, sFrom));
            }
            if (oToDate) {
                var sTo = oToDate.getFullYear() + "-" + 
                          String(oToDate.getMonth() + 1).padStart(2, '0') + "-" + 
                          String(oToDate.getDate()).padStart(2, '0') + "T23:59:59";
                aFilters.push(new Filter("orderDate", FilterOperator.LE, sTo));
            }
            ...
            // In V4, filter() automatically resets skip/paging
